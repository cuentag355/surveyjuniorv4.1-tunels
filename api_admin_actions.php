<?php
// api_admin_actions.php (v3.0 - Añade Toggle Academia)
...
$value = $input['value'] ?? null; // Para los toggles
...
try {
    switch ($action) {
        ...
        // --- ¡NUEVA ACCIÓN! ---
        case 'toggle_academy':
            $academy_file = 'ACADEMIA_DISABLED';
            if ($value === true) { // true = Suspender
                file_put_contents($academy_file, 'Suspendido el ' . date('c'));
                logActivity($pdo, $user['id'], $user['username'], 'Admin: Academia SUSPENDIDA');
                echo json_encode(['success' => true, 'message' => 'Academia SUSPENDIDA para usuarios.']);
            } else { // false = Reactivar
                if (file_exists($academy_file)) { unlink($academy_file); }
                logActivity($pdo, $user['id'], $user['username'], 'Admin: Academia REACTIVADA');
                echo json_encode(['success' => true, 'message' => 'Academia REACTIVADA para usuarios.']);
            }
            break;
