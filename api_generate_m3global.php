<?php
// api_generate_m3global.php (v10.2 - Fix Cache Stats)
...
if (preg_match('/[?&]survey_key=([^&]+)/i', $urls, $matches)) $survey_key = $matches[1];
if (preg_match('/[?&]user_id=([^&]+)/i', $urls, $matches)) $user_id = $matches[1];

if (!$survey_key || !$user_id) {
    http_response_code(400); echo json_encode(['success' => false, 'message' => 'Faltan parámetros (survey_key o user_id).']); exit;
}

$jumper = "https://hub.m3globalresearch.com/survey_complete.cfm?survey_key=" . urlencode($survey_key) . "&user_id=" . urlencode($user_id);
...
logActivity($pdo, $user['id'], $user['username'], 'Generar M3Global Exitoso', "Key: {$survey_key}");

echo json_encode([
    'success' => true,
    'message' => "¡Jumper M3Global Generado!",
    'jumper' => $jumper,
    'subid' => $survey_key,
    'added_by' => 'Sistema',
    'pais' => 'N/A'
